...
import cn.darkranger.log.log4j_lib.LogUtil;

public class RefundInfActivity extends BaseActivity{
	private static Logger log = Logger.getLogger(RefundInfActivity.class);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(null!=intent){
			...
			String rc = CheckUtil.chkRefund(mContext, uri);
			if(rc.equals("")){
				...
				requestObj.setRefundTxnNo(uri.getQueryParameter("RefundTxnNo")==null?"":uri.getQueryParameter("RefundTxnNo"));
				requestObj.setOrgMerchantID(uri.getQueryParameter("OrgMerchantID")==null?"":uri.getQueryParameter("OrgMerchantID"));
				...
				requestObj.setRefundReason(uri.getQueryParameter("RefundReason")==null?"":uri.getQueryParameter("RefundReason"));
	...
	public void onDestroy() { 
		 ...
		 /*if(ServiceUtil.getXposDriverService()!=null)
			 unbindService(ServiceUtil.getMserviceConnection());*/
		 /*if(MobileUtil.xposDriverService != null) {
	...
	Runnable runnableQuery = new Runnable(){
	    @Override
	    public void run() {
	    	 String  respContent = refundQuery();
	    	 Message msg = new Message();
		     Bundle data = new Bundle();
		     data.putString("respContent",respContent);
		     msg.setData(data);
	...
	public String refund(){
    	...
    	try {
    		...
    		postData.put("TxnType", Constant.TxnType.REFUND);
    		postData.put("PartnerID", Constant.HttpArg.PARTNER_ID);
    		postData.put("RefundTxnNo", requestObj.getRefundTxnNo());
    		...
    		postData.put("OrgMerchantID", requestObj.getOrgMerchantID());
    		postData.put("MerchantID", mid);
    		postData.put("TerminalID", tid);
    		postData.put("RefundAmt", requestObj.getRefundAmt());
    		...
    		postData.put("RefundReason", requestObj.getRefundReason());
    		...
    		log.info("扫码退货交易信息："+postData.toString());
			...
			log.info("扫码退货交易报错："+e1.getLocalizedMessage());
	...
	public String refundQuery(){
    	...
    	try {
    		JSONObject postData = new JSONObject();
			postData.put("AccTypeFlag", "1");
    		postData.put("TxnType", Constant.TxnType.REFUNDQUERY);
    		postData.put("PartnerID", Constant.HttpArg.PARTNER_ID);
    		postData.put("ChannelID", requestObj.getChannelID());
    		postData.put("OrgRefundTxnNo", requestObj.getRefundTxnNo());
    		postData.put("MerchantID", mid);
    		postData.put("TerminalID", tid);
    		postData.put("RefundAmt", requestObj.getRefundAmt());
    		postData.put("CashierID", requestObj.getCashierID());
	...
	public void setPrintText(JSONObject json){
		...
		try {
			...
			printer.addText(formatNC, json.getString("MerchantName"));
			
			printer.addText(formatLC, mContext.getString(R.string.mobile_printStr18)+":"+FormatUtil.double2StrAmt(new Double(requestObj.getRefundAmt())));
			
			printer.addText(formatSC, mContext.getString(R.string.mobile_tranSucc));

			printer.addText(formatSC, "----------------------------------------------");
			
			printer.addText(formatNL, mContext.getString(R.string.mobile_printStr16)+":"+json.getString("PayerID"));
			...
			printer.addText(formatNL, mContext.getString(R.string.mobile_printStr10)+":"+json.getString("TxnAnsTime"));
...
//      	 backIntent.putExtra("ChannelID", MobileUtil.getSoftposChannelId(backJson.getString("ChannelID"), backJson.getString("BranchChannelID")));
	...
	public String writePrintText(JSONObject json){
		...
		for (int i=0; i<2; i++) {
			...
			stringBuilder.append(formatLCStart).append(mContext.getString(R.string.mobile_printStr18) + ": -" + FormatUtil.double2StrAmt(new Double(requestObj.getRefundAmt()))).append(formatLCEnd).append(formatEnter);
